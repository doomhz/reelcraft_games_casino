...
class RcgHelper {
  ...
  request(command, data, callback){
    ...
    request(options, function (err, response, body){
      ...
      if (!response) return callback(`Empty response - ${command} - ${JSON.stringify(data)}`)
      if (response.statusCode !== 200) return callback(`Status code ${response.statusCode} != 200, message: ${response.statusMessage} - ${command} - ${JSON.stringify(data)}`)
